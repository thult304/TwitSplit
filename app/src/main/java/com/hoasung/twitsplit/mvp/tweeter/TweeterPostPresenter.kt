...
class TweeterPostPresenter : BaseMvpPresenter<MvpView>() {
    ...
    fun parseMessages(message: String) {
        val view = mvpView as TweeterPostMvpView
        view.showLoading()

        Flowable.fromCallable {
            splitMessage(message, MAX_PARTITION)

        }.subscribeOn(view.getSubscribeScheduler())
                .observeOn(view.getObserveScheduler())
                .subscribe(
                        { segments -> view.onParsedMessageSuccess(segments) },
                        { error -> view.onParsedMessageFail(error) })
    }
